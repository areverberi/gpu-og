...
__constant__ float angle_min;
__constant__ float angle_range;
__constant__ int num_scans;
...
__global__ void postProcMapK(float * mapin, int8_t * mapout, int w, int h, size_t pitch, size_t pitch_o, int w_th, int h_th);
...
void postProcMap(float * mapin, int8_t * mapout, int w, int h, size_t pitch, size_t pitch_o, int w_th, int h_th )
{
	dim3 numThr(32, 32);
	dim3 numBlocks(w/numThr.x, h/numThr.y);
    	postProcMapK <<<numBlocks, numThr>>> (mapin, mapout, w, h, pitch/sizeof(float), pitch_o/sizeof(int8_t), 1, 1);
...
__global__ void postProcMapK(float* mapin, int8_t *mapout, int w, int h, size_t pitch, size_t pitch_o, int w_th, int h_th){
	...
	for(unsigned int i=0; i<h_th; ++i)
	{
		for(unsigned int j=0; j<w_th; ++j)
		{
			...
			if(x<w && y<h)
			{
				float map_v=mapin[x+y*pitch];
				mapout[x+y*pitch_o]=(int8_t)rintf(map_v>0.0?100*map_v:map_v);
				/*if(map_v>0.5f)
					mapout[x+y*pitch_o]=100;
				else if(map_v>=0.0f)
					mapout[x+y*pitch_o]=0;
				else
					mapout[x+y*pitch_o]=-1;
				*/
			}
		}
	}
...
__device__ void getCoordsBresenham(float *coords, float * range, float * x_o, float * y_o, float * theta_o, int coord_sys=0)
{
	...
	if(threadIdx.x==0)
	{
        theta_b=*theta_o+blockIdx.x*angle_range/(num_scans-1)+angle_min;
		float s;
		float c;
		...
		x2=(int)floorf(mapW/2+(*x_o +(*range)*c)/resolution);
		y2=(int)floorf(mapH/2+(*y_o +(*range)*s)/resolution);
		delta_x=(float)(x2-x1);
		delta_y=(float)(y2-y1);
...
__global__ void computeMatchScoresK(float * x_part, float * y_part, float * theta_part, float * scan_gpu, float *map, size_t pitch, float * scores)
{
	...
	if (range<6.4f)
	{
        float theta_t=theta+threadIdx.x*angle_range/(num_scans-1)+angle_min;
		float s, c;
		__sincosf(theta_t, &s, &c);
		int x_hit=(int)floorf(mapW/2+(x+(range)*c)/resolution);
		int y_hit=(int)floorf(mapH/2+(y+(range)*s)/resolution);
...
void init(float * map, size_t * pitch, int * width, int * height, thrust::device_vector<float> & x_part, thrust::device_vector<float> & y_part, thrust::device_vector<float> & theta_part, std::vector<int> & numScans, std::vector<std::vector<float> > & scans, std::vector<float> & xs, std::vector<float> & ys, std::vector<float> & thetas, float * scores, float * x_old, float * y_old, float * theta_old)
{
    ...
    check_cuda_error(cudaMemcpyToSymbol(angle_min, &a_min, sizeof(float)));
    check_cuda_error(cudaMemcpyToSymbol(angle_range, &a_range, sizeof(float)));
    check_cuda_error(cudaMemcpyToSymbol(num_scans, &n_scans, sizeof(int)));
	dim3 numThr(32, 32);
...
int8_t * get_map( int8_t *map_gpu, int width, int height, size_t pitch)
{
	int8_t *mapsave;
	check_cuda_error(cudaMallocHost(&mapsave, width*height*sizeof(int8_t)));
	//printf("og - malloced\n");
	size_t pitchSave=sizeof(int8_t)*width;
	printf("%u %u %u %u\n", width, height, pitch, pitchSave);
	check_cuda_error(cudaMemcpy2D(mapsave, pitchSave, map_gpu, pitch, width*sizeof(int8_t), height, cudaMemcpyDeviceToHost));
	//printf("og - memcpyd\n");
	return mapsave;
}
int resample(thrust::device_vector<float> & x_part, thrust::device_vector<float> & y_part, thrust::device_vector<float> & theta_part, thrust::device_ptr<float> & weights, thrust::device_vector<float> & resampling_vector, thrust::device_vector<int> & resampled_indices)
{
	...
	thrust::device_vector<float> weightsSum(NUM_PARTICLES);
	thrust::inclusive_scan(weights, weights+NUM_PARTICLES, weightsSum.begin());
	thrust::transform(weightsSum.begin(), weightsSum.end(), thrust::make_constant_iterator(weightsSum[NUM_PARTICLES-1]), weightsSum.begin(), thrust::divides<float>());
	...
	thrust::lower_bound(weightsSum.begin(), weightsSum.end(), resampling_vector.begin(), resampling_vector.end(), resampled_indices.begin());
...
void setMapParams(int width, int height, float res)
{
	check_cuda_error(cudaMemcpyToSymbol(mapW, &width, sizeof(int)));
        check_cuda_error(cudaMemcpyToSymbol(mapH, &height, sizeof(int)));
        check_cuda_error(cudaMemcpyToSymbol(resolution, &res, sizeof(float)));
	int mw, mh;
	float r;
	check_cuda_error(cudaMemcpyFromSymbol(&mw, mapW, sizeof(mapW), 0, cudaMemcpyDeviceToHost));
	check_cuda_error(cudaMemcpyFromSymbol(&mh, mapH, sizeof(mapH), 0, cudaMemcpyDeviceToHost));
	check_cuda_error(cudaMemcpyFromSymbol(&r, resolution, sizeof(resolution), 0, cudaMemcpyDeviceToHost));
	//printf("map p:%d %d %f\n", mw, mh, r);
}
void setScanParams(float a_min, float a_range, int n_scans, float r_max)
{
	check_cuda_error(cudaMemcpyToSymbol(angle_min, &a_min, sizeof(float)));
        check_cuda_error(cudaMemcpyToSymbol(angle_range, &a_range, sizeof(float)));
        check_cuda_error(cudaMemcpyToSymbol(num_scans, &n_scans, sizeof(int)));
        check_cuda_error(cudaMemcpyToSymbol(range_max, &r_max, sizeof(float)));
	float am, ar, rm;
	int ns;
	check_cuda_error(cudaMemcpyFromSymbol(&am, angle_min, sizeof(angle_min), 0, cudaMemcpyDeviceToHost));
	check_cuda_error(cudaMemcpyFromSymbol(&ar, angle_range, sizeof(angle_range), 0, cudaMemcpyDeviceToHost));
	check_cuda_error(cudaMemcpyFromSymbol(&ns, num_scans, sizeof(num_scans), 0, cudaMemcpyDeviceToHost));
	check_cuda_error(cudaMemcpyFromSymbol(&rm, range_max, sizeof(range_max), 0, cudaMemcpyDeviceToHost));
	//printf("scan p:%d %f %f %f\n", ns, am, ar, rm);
